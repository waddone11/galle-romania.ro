...
>
    <section class="bg-forest text-mist-warm py-16">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p class="mt-4 text-lg text-mist">Portofoliu Galle Silva — lucrari pentru primarii, proprietari privati si companii.</p>
        </div>
    </section>
    <section class="py-16">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($proiecte as $p)
                <a href="/proiecte/{{ $p->slug }}" class="block group">
                    ...
                    <h2 class="font-display text-xl font-semibold mt-1 group-hover:text-mint">{{ $p->getTranslation('titlu', 'ro') }}</h2>
                    <p class="text-sm text-forest-dark/70 mt-2 line-clamp-2">{{ $p->getTranslation('descriere', 'ro') }}</p>
                    @if($p->locatie)
                        <p class="text-xs text-forest-dark/50 mt-2">{{ $p->locatie }}</p>
                    @endif
                </a>
            @empty
                <p class="col-span-full text-center text-forest-dark/60">Nu sunt proiecte publicate inca.</p>
            @endforelse
        </div>
    </section>
</x-layouts.app>
